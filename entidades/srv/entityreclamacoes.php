...
// **********************************
//
// RETURN ALL CLAIMS OF A GIVEN ENTITY
//
//
// **********************************
...
$query = "
SELECT
  ID_RECLAMACAO AS ID_RECLAMACAO,
  NID,
  ...
  GetClassConteudoReclamacaoStr(ID_RECLAMACAO) AS CLASSIFICACAO_CONTEUDO,
  NUMERO_OFICIOS_DENUNCIANTE AS N_OFICIOS_PARA_DENUNCIANTE,
  NUMERO_OFICIOS_AUTORIDADE AS N_AUTORIDADES_OFICIOS, 
  GetOficiosStr(ID_RECLAMACAO,'R') AS OFICIOS_PARA_DENUNCIANTE, 
  GetProdutosStr(ID_RECLAMACAO) AS PRODUTOS,
  
  GetInfraccoesReclamacoesStr(ID_RECLAMACAO) AS INFRACCOES,
  
  GetCorrespFiscStr(ID_RECLAMACAO) AS FISCALIZACOES_RELACIONADAS,
  GetCorrespProcessosStr(ID_RECLAMACAO) AS PROCESSOS_RELACIONADOS,
...
LEFT JOIN CORRESP_ACTIVIDADES CA ON (CA.CORRESP_ID_CORRESP = DEN.ID_RECLAMACAO)
...
foreach ($result->fetchAll(PDO::FETCH_ASSOC) as $row) {
    ...
    $json[]= array(
      'ID_RECLAMACAO' => array_key_exists('ID_RECLAMACAO',$row)?$row['ID_RECLAMACAO']:"-",
